function DistLoadFlowSolution=powerflow(x,t,ng,nc)
...
QC = zeros(1,nb);
NGpq = round(x(1:ng));
NCpq = round(x(ng+1:ng+nc));
%sg = round(x(3*ng+2*nc+1:4*ng+2*nc));
PG(NGpq)= x((ng+nc+1):(2*ng+nc));
%for j = 1:ng
%    sn = NGpq(j);
%    if sg(j)==2 
%        QG(sn)= x(2*ng+nc+j);
...
%    end
%end
QG(NGpq)=x((2*ng+nc+1):(3*ng+nc));
QC(NCpq)= x((3*ng+nc+1):(3*ng+2*nc));
% As per ELM data
VB=bus(:,VM);
Vmm=abs(VB);
P=(Ploaddata(t,:)-PG*1e3)'/(baseMVA*1000); % in p.u. as P in kW
% P=P.*(Vmm.^2);
Q=((Qloaddata(t,:)-(QG+QC)*1e3)')./(baseMVA*1000); % in p.u. as Q in kVAr
% Q=Q.*(Vmm.^2);
...
iter=300; % Can be changed (if taking more time)
for i=1:iter
     %%Backward Sweep
    ...
    delV=DLF*I/sqrt(3); % Voltage drop w.r.t. substation bus
